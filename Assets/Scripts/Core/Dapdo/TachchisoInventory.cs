...
public class TachchisoInventory : MonoBehaviour
{
    public static TachchisoInventory Singleton;
    ...
    //public TachepsaoUI tachepsaoUI;
        ...
        for (int i = 0; i < Inventory.Singleton.items.Count; i++)
        {
            ...
            if (Inventory.Singleton.items[i].item != null)
            {
                ...
                if (itemProfile.Click != null)
                {
                    ...
                    itemProfile.Click.onClick.AddListener(() => itemProfile.OnItemClickedTachsaos(Inventory.Singleton.items[index]));
