...
{
    public static ShopInventory Singleton;
        ...
        for (int i = 0; i < Inventory.Singleton.items.Count; i++)
        {
            ...
            if (Inventory.Singleton.items[i].item != null)
            {
                ...
                if (itemProfile.Click != null)
                {
                    ...
                    itemProfile.Click.onClick.AddListener(() => itemProfile.OnItemClicked(Inventory.Singleton.items[index]));
                }
    ...
    public void Sell()
    {
        Inventory.Singleton.SellItem(vitriitem, 1,giaban);
        UpdateUI();
    }
    public void Equipment()
    {
        EquipmentManager.Singleton.EquipItem(item[0], EquipmentManager.Singleton.slotNameMac.ToString());
        EquipmentUIManager.Singleton.UpdateEquipmentUI(); // Làm mới UI
        UpdateItem();
